...
// Returns 0 on success, -1 on error.
 int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    /* Step 2: build header  e.g. "blob 16\0" */
    char header[64];
    int hlen = snprintf(header, sizeof(header), "%s %zu", type_str, len) + 1;
    /* hlen includes the '\0' terminator — snprintf returns chars written
       without '\0', so +1 gives us the null byte's position */

    /* Step 3: allocate full object buffer = header + data */
    size_t full_len = (size_t)hlen + len;
    uint8_t *full = malloc(full_len);
    if (!full) return -1;
    memcpy(full, header, hlen);          /* header including '\0' */
    memcpy(full + hlen, data, len);      /* raw file data */

    /* Step 4: compute SHA-256 of full object */
    compute_hash(full, full_len, id_out);

    /* Step 5: deduplication — if already stored, skip write */
    if (object_exists(id_out)) { free(full); return 0; }

    /* Step 6: build shard dir path  e.g. ".pes/objects/2f/" */
    char hex[HASH_HEX_SIZE + 1];
    hash_to_hex(id_out, hex);
    char dir[512];
    snprintf(dir, sizeof(dir), "%s/%.2s", OBJECTS_DIR, hex);
    mkdir(dir, 0755);   /* safe to call even if dir exists */

    /* Step 7: write to a temp file in the same shard dir */
    char tmp_path[512];
    ...
    /* Step 8: fsync to flush to disk before rename */
    fsync(fd);
    close(fd);
    free(full);
...
int object_read(const ObjectID *id, ObjectType *type_out,
    ...
    if (memcmp(computed.hash, id->hash, HASH_SIZE) != 0) {
        free(buf); return -1;   /* file is corrupt */
    }
